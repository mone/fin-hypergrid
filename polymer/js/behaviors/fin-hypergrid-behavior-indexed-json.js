...
(function() {
    ...
    Polymer({ /* jslint ignore:line */
        ...
        setValue: function(x, y, value) {
            var fields = this.getFields();
            this.data[this.rows[y]][fields[x]] = value;
        },
        ...
        applySorts: function() {
            ...
            var sorts = this.tableState.sorted;
            //this.grid.clearSelections(); //TODO ?
            // var colIndexes = state.columnIndexes; //TODO what's column indexes?
            if (!sorts) {
                return;
            }
            ...
            this.rows.sort(function(v1, v2) {
                for (var c = 0; c < columnsToSort.length; c++) {
                    var a, b;
                    var column = columnsToSort[c];
                    var field = fields[column];
                    var sortType = sorts[column];

                    if (sortType === 1) {
                        a = that.data[v2][field];
                        b = that.data[v1][field];
                    } else if (sortType === 2) {
                        a = that.data[v1][field];
                        b = that.data[v2][field];
                    } else {
                        //TODO I don't know what does it mean
                        return 0;
                    }
        ...
        removeRow: function(key) {
            var y = this.rows.findIndex(function(element) {
                return key === element;
            });
